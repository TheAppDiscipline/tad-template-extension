...
## Backend / Sidecar Pattern

This extension template does not import Supabase or Firebase directly. If the extension needs auth, billing, or cross-device sync, put that backend in a sidecar web app created from `tad-template-web`, then call the sidecar API from the extension with `fetch`.

Keep browser credentials in `browser.storage.local`; never ship service-role keys or server API keys inside the extension bundle.
