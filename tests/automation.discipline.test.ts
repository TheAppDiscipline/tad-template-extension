...
describe('discipline:step4-origin (fail-loud)', () => {
  ...
  it('chooses reentry only when the outcome is terminal and the completion gate passed', () => {
    ...
    const partial = resolveOrigin({ 'STEP_4_EXECUTION_PACKET.md': EXEC_VALIDATED, 'SLICE_COMPLETION_PACKET.md': COMPLETION_PARTIAL })
    expect(partial.exit, partial.raw).toBe(2)
    expect(partial.json.status).toBe('invalid')
    expect(partial.json.reason).toMatch(/partial.*leaves the slice open/i)
  ...
  it('detectNext authorizes a Step 4 advance only when the origin is coherent', () => {
    ...
    expect(detect({ 'STEP_4_EXECUTION_PACKET.md': EXEC_VALIDATED, 'SLICE_COMPLETION_PACKET.md': COMPLETION_PARTIAL })).toBe(null)
