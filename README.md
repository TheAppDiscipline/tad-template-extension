...
**Template release prepared:** v1.0.0, not published yet. When you create a real extension from this template, keep strict semver in `package.json`; the Chrome Web Store and Firefox AMO require uploading a higher version than the previous one on each upload.
...
The optional `entrypoints/content.ts` scaffold is excluded from Chromium and Firefox builds. To enable it, first set `matches` to the exact URLs your feature needs, then remove `exclude`. Empty match lists prevent Chromium from loading the extension; broad host access is not needed for the starter popup.

`npm run gate:visual` builds and loads the real extension in Playwright's bundled Chromium. It requires a registered background service, a storage message round trip, and the rendered popup. Missing builds, missing registration and broken popup code fail the check. Install its browser with `npm run e2e:install`. Firefox MV3 has a separate build; this Chromium test does not verify Firefox runtime.
