...
- If it does not exist, mark a BLOCKER: "PROFILE=LAUNCH+ requires scorecard.yaml. Create it from the scorecard template in the vault (sold separately)."
...
- The report must be interpretable by a non-programmer: use glossary terms (in the vault, sold separately), not internal tooling jargon.
