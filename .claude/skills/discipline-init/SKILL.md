...
NOTE: the skill does not pick the lane for the user. If you are unsure which lane to use, first run `/discipline-step0a` (which produces IDEA_VALIDATION_PACKET) and/or consult the lane selection guide in the vault (sold separately). Then invoke this skill with the confirmed lane.
...
If you are a non-programmer, see the step-by-step setup instructions in the vault (sold separately).
```
...
Full skills catalog: in The App Discipline vault (sold separately).
