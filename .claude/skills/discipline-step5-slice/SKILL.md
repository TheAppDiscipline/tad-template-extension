...
2. Buscar fix directo en la guía de errores comunes del gate (en el vault, producto aparte).
...
Escribir `.discipline/packets/SLICE_COMPLETION_PACKET.md` con la estructura canonica del SLICE_COMPLETION_PACKET (documentada en el vault, producto aparte):
...
Si `Deploy signal != not_ready`, generar tambien `.discipline/packets/DEPLOY_READINESS_PACKET.md` con la estructura del DEPLOY_READINESS_PACKET (documentada en el vault, producto aparte).
...
- Si patches pendientes y `discipline:patch` falla: parar, reportar conflicto, redirigir a la guía de aplicar patch blocks (en el vault, producto aparte).
