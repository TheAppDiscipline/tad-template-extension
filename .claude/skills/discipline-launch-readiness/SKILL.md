...
1. The skill checks that `.discipline/scorecard.yaml` exists. If not, it offers to generate the skeleton from the scorecard template (documented in The App Discipline vault, sold separately).
...
1. Define the criteria in `.discipline/scorecard.yaml` (template in the vault, sold separately).
...
Options:
1. Generate the skeleton now (I create the file with the canonical template from the vault, sold separately; you fill it in with evidence).
...
If it fails because scorecard.yaml is malformed: show the parser error and point to the scorecard schema in the vault (sold separately).
...
- `validate-scorecard.ts` does not exist: the template is not post-Wave 3.1. Suggest updating to the current version or installing the script manually (see the vault, sold separately).
